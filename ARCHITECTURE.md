...
- `/artifacts`, `/memories`, `/chief-of-staff`, and `/entities` are now shipped bounded review workspaces that expose existing artifact, memory (including open-loop review), chief-of-staff priority, and entity read seams with explicit live/fixture/unavailable modes.
...
- `apps/api` exposes FastAPI endpoints for:
  - continuity and response generation: `/healthz`, `POST /v0/threads`, `GET /v0/threads`, `GET /v0/threads/{thread_id}`, `GET /v0/threads/{thread_id}/sessions`, `GET /v0/threads/{thread_id}/events`, `GET /v0/threads/{thread_id}/resumption-brief`, `GET /v0/chief-of-staff`, `POST /v0/context/compile`, `POST /v0/responses`
...
- `apps/web` exposes the current operator shell:
  ...
  - `/chief-of-staff`: deterministic priority dashboard with ranked priorities, rationale visibility, trust-aware confidence posture, and one recommended next action
...
- Review workspaces are covered at route level in `apps/web/app/artifacts/page.test.tsx`, `apps/web/app/memories/page.test.tsx`, `apps/web/app/chief-of-staff/page.test.tsx`, and `apps/web/app/entities/page.test.tsx`, with matching component and API-client coverage under `apps/web`.
