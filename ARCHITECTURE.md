...
- `/artifacts`, `/memories`, `/chief-of-staff`, and `/entities` are now shipped bounded review workspaces that expose existing artifact, memory (including open-loop review), chief-of-staff priority plus follow-through supervision, and entity read seams with explicit live/fixture/unavailable modes.
...
- `apps/web` exposes the current operator shell:
  ...
  - `/chief-of-staff`: deterministic priority dashboard plus deterministic follow-through supervision with overdue/stale/slipped groupings, explicit escalation posture, and draft-only follow-up artifact visibility
