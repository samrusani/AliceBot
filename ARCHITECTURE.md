...
3. Persist Gmail secrets only in the dedicated `gmail_account_credentials` protected credential seam bound to the same visible user/account ownership scope, using either a narrow access-token-only shape or a refresh-token-capable credential shape with expiry metadata.
