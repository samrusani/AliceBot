...
- `apps/api` exposes FastAPI endpoints for:
  - continuity and response generation: `/healthz`, `POST /v0/threads`, `GET /v0/threads`, `GET /v0/threads/{thread_id}`, `GET /v0/threads/{thread_id}/sessions`, `GET /v0/threads/{thread_id}/events`, `GET /v0/threads/{thread_id}/resumption-brief`, `GET /v0/chief-of-staff`, `POST /v0/chief-of-staff/recommendation-outcomes`, `POST /v0/context/compile`, `POST /v0/responses`
...
- `apps/web` exposes the current operator shell:
  ...
  - `/chief-of-staff`: deterministic priority dashboard plus deterministic follow-through supervision, deterministic preparation/resumption artifacts (`preparation_brief`, `what_changed_summary`, `prep_checklist`, `suggested_talking_points`, `resumption_supervision`), and deterministic weekly review/outcome-learning artifacts (`weekly_review_brief`, `recommendation_outcomes`, `priority_learning_summary`, `pattern_drift_summary`) with explicit provenance/rationale visibility, trust-aware confidence posture, and draft-only follow-up artifact visibility
