...
- `apps/web` exposes the current operator shell:
  ...
  - `/chief-of-staff`: deterministic priority dashboard plus deterministic follow-through supervision and deterministic preparation/resumption artifacts (`preparation_brief`, `what_changed_summary`, `prep_checklist`, `suggested_talking_points`, `resumption_supervision`) with explicit provenance/rationale visibility, trust-aware confidence posture, and draft-only follow-up artifact visibility
