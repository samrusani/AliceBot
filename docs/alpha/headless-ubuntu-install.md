...
(`v0.11.0`) for an immutable deployment.
...
For an immutable install after a release tag is published, replace `--branch main` with `--tag <release-tag>` (currently `--tag v0.11.0`).
