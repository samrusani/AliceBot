"""SQLite extraction accounting and source-carrier persistence."""
...
def list_memories_for_source_chunk(
    ...
    rows = self._fetch_all(
        ...
        WHERE user_id = ?
          AND deleted_at IS NULL
          AND json_extract(metadata_json, '$.source_chunk_id') = ?
          AND json_extract(
            metadata_json,
...
def write_occurrence_memory_metadata(
    ...
    *,
    ...
    cursor = self._execute(
        """
        SELECT metadata_json AS raw_metadata_json
        FROM memories
        WHERE id = ?
          AND user_id = ?
          AND deleted_at IS NULL
    ...
    raw_mapping = (
        dict(raw_row)
        if isinstance(raw_row, (Mapping, sqlite3.Row))
        else {"raw_metadata_json": cast(tuple[object, ...], raw_row)[0]}
    )
    current_text = str(raw_mapping["raw_metadata_json"])
    parsed = json.loads(current_text)
    if not isinstance(parsed, dict):
        raise ContinuityStoreInvariantError("current memory metadata is not an object")
    current_metadata = cast(JsonObject, parsed)
    if expected_metadata_json is not None and current_metadata != dict(expected_metadata_json):
    ...
    if not changed_keys:
        return self._fetch_one(
            "write_occurrence_memory_metadata unchanged",
            f"""
            SELECT {", ".join(MEMORY_COLUMNS)}
            FROM memories
    ...
    replacement_text = _canonical_json_text(replacement)
    cursor = self._execute(
        f"""
        UPDATE memories
        SET metadata_json = ?
    ...
    if raw_updated is None:
        raise ContinuityStoreInvariantError("write_occurrence_memory_metadata lost its metadata CAS")
    row = _decode_cursor_row(self, cursor, raw_updated)
    chunk_ids = sorted(
...
def list_occurrence_claims_for_source_chunk(
    ...
    *,
    ...
    return self._fetch_all(
        ...
        LIMIT ?
        """,
        (self.user_id, str(source_chunk_id), limit),
    )
...
def list_accepted_occurrence_extraction_dispositions_for_claims(
    ...
    *,
    ...
    return self._fetch_all(
        ...
        WHERE disposition.user_id = ?
          AND disposition.review_status = 'accepted'
          AND source.deleted_at IS NULL
          AND EXISTS (
            SELECT 1
            FROM json_each(disposition.claim_ids) AS bound_claim
            WHERE CAST(bound_claim.value AS TEXT)
              IN ({placeholders})
          )
        ORDER BY disposition.source_chunk_id ASC, disposition.id ASC
        LIMIT ?
        """,
        (self.user_id, *ids, limit),
    )
...
def _current_extraction_chunk(
    ...
    return self._fetch_optional_one(
        ...
        WHERE chunk.user_id = ?
          ...
          AND source.deleted_at IS NULL
        """,
        (self.user_id, str(source_chunk_id)),
    )
...
def _validate_extraction_references(
    ...
    *,
    ...
    found_occurrence_ids: set[str] = set()
    for offset in range(0, len(occurrence_ids), 400):
        batch = list(occurrence_ids[offset : offset + 400])
        ...
        rows = self._fetch_all(
            ...
            WHERE unit.user_id = ?
              AND unit.id IN ({placeholders})
              {review_predicate}
              AND evidence.source_chunk_id = ?
              AND evidence.evidence_role = 'supports'
              AND evidence.review_status IN ('candidate', 'accepted')
            """,
            (self.user_id, *batch, str(source_chunk_id)),
        )
        found_occurrence_ids.update(str(row["id"]) for row in rows)
    if found_occurrence_ids != set(occurrence_ids):
...
def _current_disposition_memory_facts(
    ...
    rows = self._fetch_all(
        ...
        """,
        (str(source_chunk_id), self.user_id),
    )
...
def invalidate_occurrence_extraction_dispositions(
    ...
    *,
    ...
    params: list[object] = [self.user_id, str(source_chunk_id)]
    extractor_predicate = ""
    if extractor is not None:
        extractor_predicate = "AND extractor_version = ?"
    ...
    for current in rows:
        ...
        invalidated.append(
            self._fetch_one(
                "invalidate occurrence extraction disposition",
                f"""
                SELECT {_column_sql(OCCURRENCE_EXTRACTION_DISPOSITION_COLUMNS)}
                FROM occurrence_extraction_dispositions
...
def record_occurrence_extraction_disposition(
    ...
    *,
    ...
    existing = self._fetch_optional_one(
        ...
        """,
        (self.user_id, str(source_chunk_id), snapshot_sha256, extractor),
    )
    if existing is not None:
        mismatches = [field for field, value in expected.items() if not _same_value(existing.get(field), value)]
        if not mismatches:
            if existing.get("review_status") == "candidate":
                invalidate_occurrence_coverage(
                    self,
        ...
        cursor = self._execute(
            ...
            (
                disposition,
                _json_list_text(predicates),
                _json_list_text(claims),
                _json_list_text(occurrences),
                _json_object_text(metadata),
                _utc_now_iso(),
                self.user_id,
                str(existing["id"]),
                int(cast(int, existing["review_version"])),
            ),
        )
        if cursor.rowcount != 1:
            raise ContinuityStoreInvariantError("extraction disposition replacement lost its lifecycle CAS")
        row = self._fetch_one(
            "replace occurrence extraction disposition",
            f"""
            SELECT {_column_sql(OCCURRENCE_EXTRACTION_DISPOSITION_COLUMNS)}
            FROM occurrence_extraction_dispositions
            WHERE user_id = ? AND id = ?
    ...
    row = self._fetch_one(
        "record occurrence extraction disposition",
        f"""
        SELECT {_column_sql(OCCURRENCE_EXTRACTION_DISPOSITION_COLUMNS)}
        FROM occurrence_extraction_dispositions
        WHERE user_id = ? AND id = ?
        """,
        (self.user_id, disposition_id),
    )
    self._append_mutation_event(
...
def review_occurrence_extraction_disposition(
    ...
    *,
    ...
    current = self._fetch_optional_one(
        ...
        WHERE user_id = ? AND id = ?
        """,
        (self.user_id, str(disposition_id)),
    )
    if current is None:
    ...
    cursor = self._execute(
        ...
        (
            ...
            _json_list_text(current["predicate_keys"]),
            _json_list_text(current["claim_ids"]),
            _json_list_text(current["occurrence_ids"]),
            _json_object_text(current["metadata_json"]),
        ),
    )
    if cursor.rowcount != 1:
        raise ContinuityStoreInvariantError("extraction disposition review lost its lifecycle CAS")
    row = self._fetch_one(
        "review occurrence extraction disposition",
        f"""
        SELECT {_column_sql(OCCURRENCE_EXTRACTION_DISPOSITION_COLUMNS)}
        FROM occurrence_extraction_dispositions
        WHERE user_id = ? AND id = ?
...
def summarize_occurrence_extraction_accounting(
    ...
    *,
    ...
    if source_ids is not None and not normalized_source_ids:
        ...
        placeholders = ", ".join("?" for _value in normalized_source_ids)
        chunks = self._fetch_all(
            ...
            ORDER BY chunk.id ASC
            """,
            (self.user_id, *normalized_source_ids),
        )
    else:
        chunks = self._fetch_all(
            """
    ...
    dispositions: list[VNextRow] = []
    if chunk_ids:
        placeholders = ", ".join("?" for _value in chunk_ids)
        dispositions = self._fetch_all(
            f"""
            SELECT {_column_sql(OCCURRENCE_EXTRACTION_DISPOSITION_COLUMNS)}
            FROM occurrence_extraction_dispositions
            WHERE user_id = ?
            ...
            """,
            (self.user_id, extractor, *chunk_ids),
        )
...
def list_occurrence_evidence_for_units(
    ...
    *,
    ...
    placeholders = ", ".join("?" for _value in ids)
    stable_as_of = _iso_or_none(as_of or datetime.now(UTC))
    return self._fetch_all(
        f"""
        SELECT
        ...
        WHERE evidence.user_id = ?
          AND evidence.occurrence_id IN ({placeholders})
          AND unit.review_status = 'accepted'
          AND unit.identity_status = 'resolved'
