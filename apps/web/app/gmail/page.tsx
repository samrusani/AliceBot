import { GmailAccountConnectForm } from "../../components/gmail-account-connect-form";
import { GmailAccountDetail } from "../../components/gmail-account-detail";
...
import {
  getFixtureGmailAccount,
  gmailAccountFixtures,
...
function resolveSelectedAccountId(requestedAccountId: string, items: GmailAccountRecord[]) {
  if (!items.length) {
    return "";
  }
...
export default async function GmailPage({
  ...
  let accounts = gmailAccountFixtures;
  let accountListSummary = gmailAccountListSummaryFixture;
  let accountListSource: ApiSource = "fixture";
  let accountListUnavailableReason: string | undefined;

  if (liveModeReady) {
    try {
      const payload = await listGmailAccounts(apiConfig.apiBaseUrl, apiConfig.userId);
      accounts = payload.items;
      accountListSummary = payload.summary;
      accountListSource = "live";
    } catch (error) {
      accountListUnavailableReason =
        error instanceof Error ? error.message : "Gmail account list could not be loaded.";
    }
  }
  ...
  let selectedAccountSource: ApiSource | "unavailable" | null = selectedAccount
    ? accountListSource
    : null;
  ...
  if (selectedFromList && liveModeReady && accountListSource === "live") {
    try {
      const payload = await getGmailAccountDetail(
      ...
      const fixtureAccount = getFixtureGmailAccount(selectedFromList.id);
      if (fixtureAccount) {
      ...
      selectedAccountUnavailableReason =
        error instanceof Error ? error.message : "Selected Gmail account detail could not be loaded.";
    }
  }
  ...
  return (
    <div className="page-stack">
      <PageHeader
        ...
        description="Review connected accounts first, inspect one selected account second, then run explicit connect or single-message ingestion actions."
        meta={
