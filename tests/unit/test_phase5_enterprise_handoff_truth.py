...
def test_phase5_handoff_package_and_owner_boundaries() -> None:
    ...
    assert "5.1.c OWNER GATE OPEN" in review
    assert "5.4 OWNER GATE OPEN" in review
    assert "no remaining P0-P3" in review
    for name in documents:
        assert "NO-GO pending owner-only gates" in documents[name], name
        assert "Stage A" in documents[name], name
        assert "Stage B" in documents[name], name
        assert "5.1.c OWNER GATE OPEN" in documents[name], name
        assert "5.4 OWNER GATE OPEN" in documents[name], name
...
def test_phase5_versions_and_protected_scope_are_bound_to_the_carrier() -> None:
    ...
    prior_handoff_changes = _git("diff", "--name-only", BASE, "--", "docs/handoff").stdout.decode().splitlines()
...
def test_phase5_integrated_carrier_is_ancestry_and_content_bound() -> None:
    ...
    if _git("cat-file", "-e", f"{BASE}^{{commit}}", check=False).returncode != 0:
        pytest.skip("full history unavailable; Phase 5 ops workflow runs this test with fetch-depth 0")
