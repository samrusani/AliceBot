...
def test_retrieval_evaluation_summary_is_deterministic_and_fixture_backed() -> None:
    ...
    assert payload_first["summary"]["fixture_count"] == 3
    assert payload_first["summary"]["evaluated_fixture_count"] == 3
    ...
    assert all(fixture["precision_at_k"] == 1.0 for fixture in payload_first["fixtures"])
