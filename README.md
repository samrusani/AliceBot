...
- `apps/api` now also ships Phase 7 Sprint 25 chief-of-staff seams:
  - `GET /v0/chief-of-staff` returns a deterministic priority brief with ranked items, explicit posture labels (`urgent`, `important`, `waiting`, `blocked`, `stale`, `defer`), provenance-backed rationale fields, trust-aware confidence posture, and deterministic recommended next action.
  - ranking composition reuses shipped continuity + trust inputs (`continuity/recall`, `continuity/open-loops`, `continuity/resumption-brief`, `memories/trust-dashboard`) without widening connector or side-effect scope.
...
- `/chief-of-staff` is the Phase 7 Sprint 25 dashboard surface for deterministic priority ranking, visible rationale/provenance, trust-aware confidence posture, and one deterministic next-action recommendation.
