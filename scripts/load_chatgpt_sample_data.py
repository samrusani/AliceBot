...
DEFAULT_SOURCE_PATH = REPO_ROOT / "fixtures" / "importers" / "chatgpt" / "workspace_v1.json"


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Import ChatGPT sample export data into Alice continuity objects."
    )
    parser.add_argument(
        "--source",
        default=os.getenv("CHATGPT_SAMPLE_DATA_PATH", str(DEFAULT_SOURCE_PATH)),
        help="Path to a ChatGPT export JSON file or directory.",
    )
    parser.add_argument(
        "--database-url",
    ...
    parser.add_argument(
        ...
        help="User ID to own imported ChatGPT data.",
    )
    parser.add_argument(
        "--user-email",
        default=os.getenv("ALICEBOT_IMPORT_USER_EMAIL", "chatgpt-sample@example.com"),
        help="Email for auto-created user when --user-id is not found.",
    )
    parser.add_argument(
        "--display-name",
        default=os.getenv("ALICEBOT_IMPORT_USER_DISPLAY_NAME", "ChatGPT Sample User"),
