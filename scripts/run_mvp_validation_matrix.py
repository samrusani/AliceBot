...
def build_validation_matrix_steps(*, python_executable: str | None = None) -> list[MatrixStep]:
    ...
    return [
        MatrixStep(
            ...
            command=(resolved_python, "scripts/run_mvp_readiness_gates.py"),
...
def _print_step_results(step_results: list[MatrixStepResult]) -> None:
    print("MVP validation matrix results:")
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run deterministic MVP validation matrix over readiness prerequisite, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if exit_code == 0:
        print("MVP validation matrix result: PASS")
    else:
        print("MVP validation matrix result: NO_GO")
    return exit_code
