...
DEFAULT_SOURCE_PATH = REPO_ROOT / "fixtures" / "importers" / "markdown" / "workspace_v1.md"


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Import markdown sample workspace data into Alice continuity objects."
    )
    parser.add_argument(
        "--source",
        default=os.getenv("MARKDOWN_SAMPLE_DATA_PATH", str(DEFAULT_SOURCE_PATH)),
        help="Path to a markdown file or directory.",
    )
    parser.add_argument(
        "--database-url",
    ...
    parser.add_argument(
        ...
        help="User ID to own imported markdown data.",
    )
    parser.add_argument(
        "--user-email",
        default=os.getenv("ALICEBOT_IMPORT_USER_EMAIL", "markdown-sample@example.com"),
        help="Email for auto-created user when --user-id is not found.",
    )
    parser.add_argument(
        "--display-name",
        default=os.getenv("ALICEBOT_IMPORT_USER_DISPLAY_NAME", "Markdown Sample User"),
...
def main() -> int:
    ...
    with user_connection(args.database_url, user_id) as conn:
        ...
        summary = import_markdown_source(
            store,
            user_id=user_id,
            source=source_path,
        )
